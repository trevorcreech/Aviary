(function () {
  ...
  function deleteModalRecording(button) {
    ...
    if (adminAccessState === 'locked') {
      window.alert('Unlock admin controls from the menu before deleting a false positive.');
      return;
    }
    if (!window.confirm('Delete this false positive?\n\nThe recording will be moved to a private, recoverable quarantine.')) return;

    row.classList.add('deleting');
    button.disabled = true;
    adminFetch('./avian/api/recording-delete.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Avian-Action': '1' },
      ...
      if (!adminAuthCancelled(error)) {
        window.alert(error && error.message ? error.message : 'Unable to delete recording.');
      }
    });
  }
  ...
  modalRecordings.addEventListener('click', function (ev) {
    ...
    var deleteButton = ev.target.closest('.rec-delete');
    if (deleteButton) {
      deleteModalRecording(deleteButton);
      return;
    }
